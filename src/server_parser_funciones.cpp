...
void FuncionesParser::parse_funciones(std::vector<Pelicula>& peliculas, std::map<std::string, Sala>& salas) {
	while (std::getline(this->file, this->sala, delim)) {
		...
		if (sala_by_id != salas.end()) {
			sala_by_id->second.nueva_funcion(Funcion(this->id, *pelicula_by_title, this->fecha, this->hora));
		}
	}
}
...
/*int main(int argc, char const *argv[]) {
	...
	while(funciones.has_items()) {
		funciones.get_funcion();
	}
	return 0;
}*/
